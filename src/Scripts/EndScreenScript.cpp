...
namespace Terrasu {
	void EndScreenScript::OnCreate(){
		...
		ent.GetComponent<TransformComponent>().Scale = {0.1f,0.1f ,0.1f };
		ent.GetComponent<TransformComponent>().Translation.x -= 0.5f;
	...
	void EndScreenScript::OnUpdate(float dt){
		...
		waitTime += dt;
		GetComponent<SpriteComponent>().material.uniforms[2].data.w += dt * 0.5f;
		if (waitTime > 3)
		{
			waitTime -= 99999;
			...
			auto ent = Instantiate<TextUIComponent>("EndGame");
			ent.GetComponent<TransformComponent>().Scale = { 0.05f,0.05f ,0.05f };
			ent.GetComponent<TransformComponent>().Translation.x -= 0.5f;
			ent.GetComponent<TransformComponent>().Translation.y -= 0.25f;
			GameOverText = &ent.GetComponent<TextUIComponent>();
