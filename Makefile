...
dependencies:
	...
	cmd /c """${EMSDKpath}"" && cd 3rdParty\bgfx\.build\projects\gmake-wasm && emmake make config=release"

	IF exist ${DIR}\3rdParty\SDL\build (echo exist) ELSE (cd 3rdParty\SDL && mkdir build)
	cmd /c """${EMSDKpath}"" && cd 3rdParty\SDL\build && emcmake cmake .. && emmake make config=release -j4"
...
emscripten:
	...
	cmd /c """${EMSDKpath}"" && cd .build\projects\gmake-wasm && emmake make config=release"
	cmd /c """${EMSDKpath}"" && cd .build\wasm\bin && emrun ."
windows:
